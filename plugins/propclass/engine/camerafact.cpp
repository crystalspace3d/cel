...
#include "plugins/propclass/engine/camerafact.h"
...
extern void EngReport (iObjectRegistry* object_reg, const char* msg, ...);
...
bool celPcCamera::Load (iCelDataBuffer* databuf)
{
  ...
  {
    EngReport (object_reg, "serialnr != CAMERA_SERIAL.  Cannot load.");
    return false;
  }
  ...
  if (!pc)
  {
    EngReport (object_reg,"Cannot load property class.");
    return false;
  }
  ...
  if (!sector)
  {
    EngReport (object_reg,"Illegal sector specified.  Cannot load.");
    return false;
  }
