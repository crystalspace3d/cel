 ...
 * The class holds a queue of items which it sorts by a priority weighting. When items
 * are inserted or removed, the queue will place / remove it from the correct position in the queue
 */
...
int celPriorityQueue :: Remove( float &fPriority )
{
    ...
    // Reshuffle the queue so that everything is back in the correct priority order again
	iqueuesize--;
...
void celPriorityQueue::SortDown()
{
    ...
    while (child < iqueuesize)
	{
		...
		if ( (rightchild < iqueuesize) && ( items[ rightchild ].Priority < items[ child ].Priority ))
		{
			child = rightchild;
		}
...
void celPriorityQueue::SortUp()
{
	...
	while (child)
	{
        ...
        {
            break;                      // child is now in the right place, so stop sorting
        }
    ...
    : celPcNavGraphRules (object_reg)
{
  ...
  Report (object_reg,"Created a Nav Graph Rules FPS object");
}
...
int celPcNavGraphRulesFPS::TraverseLink( celPcNavGraph* graph, iSector* sector, iPcNavLink* plink, iCelEntity* ent )
{
    /*
     * Check whether the entity can traverse this link
     *
     * Currently uses the celMovableConstraint on the entity to see if it can move along this
     * link or is blocked by another world entity
     */
    int iResult = -1;
    ...
    // especially when this is getting called repeatedly from NavGraph->BuildNodeGraph()
    if (previousent == NULL || ent != previousent)
    {
        Report( object_reg, "TraverseLink: Creating new entity");
        moveconst = CEL_QUERY_PROPCLASS (ent->GetPropertyClassList (), iPcMovableConstraint);
        previousent = ent;
    }
    ...
    /*
    csRef<iPcMovable> movable = CEL_QUERY_PROPCLASS (ent->GetPropertyClassList (), iPcMovable);
    ...
    if ( movable )
    {
        ...
        iResult = movable->Move( plink->GetDest()->GetPos() - plink->GetSource()->GetPos()); // move along link
    ...
    */
    ...
    {
        Report( object_reg, ".. NO MOVABLECONSTRAINT on entity!!");
...
void celPcNavGraphRulesFPS::OptimiseGraph( celPcNavGraph* graph )
{
     ...
     * 1) to remove all failed links (ie. when traverselink returned CEL_MOVE_FAIL with the 
     *    default entity
     * 2) to remove any unnecessary links that were formed from nodes that are in a straight line
     *
     * TODO: also investigate whether sorting the nodes or links would improve performance
     */

    Report( object_reg, "NavrulesFPS - OptimiseGraph");
    ...
    Report( object_reg, "Optimising graph: removed %d links", count);
    ...
    for( inode=0;inode < graph->GetNodeCount(); inode++ )
    {
        ...
        //Report( object_reg, "Checking node %d", inode);
        ...
        while( ilink1 < node->GetLinkCount() )
        {
            ...
            while( ilink2 < node->GetLinkCount() && !startagain)
            {
                ...
                if (ilink1 != ilink2) // ignore if its the same link
                {
                    ...
                    //Report( object_reg, "Checking link %d (nd %d to %d [%f,%f,%f]) vs link %d (nd %d to %d [%f,%f,%f]) of %d links - dp %f", 
                    ...
                    if (vecnode1*vecnode2 >= 0.999) // Get dot product
                    {
                        ...
                        int iremovelink = (vecnode1.SquaredNorm() > vecnode2.SquaredNorm()) ? ilink1:ilink2;

                        //Report( object_reg, "Comparing link %d (dist %f) vs link %d (dist %f) - removing %d - now %d left", 
    ...
    Report( object_reg, "Finished optimising graph: removed %d more links", count);

}

int celPcNavGraphRulesFPS::FindShortestPath( celPcNavGraph* graph, int iNodeStart, int iNodeEnd, int* &ipath )
{
    /*
     * Find shortest path between two nodes
     * This uses A* to find the shortest path between two nodes, and returns the path as an integer
     * array with the parameter ipathlength as the length of the path.
     *
     * TODO: speed up!
	...
	celPriorityQueue queue( graph->GetLinkCount() ); // Limit queue size to number of links
	...
	while ( !queue.Empty() )
	{
		...
		iCurrentNode = queue.Remove(flCurrentDistance); // returns node which gives shortest total path

		if (iCurrentNode == iNodeEnd) break; // break as soon as we get to the end of the path
		...
		{
			TestNode = pCurrentNode->GetLink( i )->GetDest(); // get destination node down this link
			...
			flOurDistance = flCurrentDistance + (pCurrentNode->GetLink( i )->GetLength());

			if( (fclosestsofar[iVisitNode] < -0.5) ||                   // new node
                (flOurDistance < fclosestsofar[iVisitNode] - 0.001 ) )  // or shortens distance
			{
                //Report( object_reg, "fl dist:%f, visit node: %d cur node: %d", flOurDistance, iVisitNode, iCurrentNode);
    ...
    #ifdef _DEBUG
//    Report( object_reg, "Number of nodes on the path: %d", iNumPathNodes);
    #endif
    ...
    //vc->Advance ();
    //Report( object_reg, "Search time: %f", vc->GetElapsedTicks ());
/*
    #ifdef _DEBUG
	...
	for ( i = 0; i < iNumPathNodes ; i++ )
	{
        ...
        Report( object_reg, "Path Node %d: %d (%f, %f, %f)", i, ipath[i], pos.x, pos.y, pos.z );
    }
    #endif
*/
    free( fclosestsofar );
...
int celPcNavGraphRulesFPS::FindNearestNode( celPcNavGraph* graph, csVector3* point, iSector* sector, iCelEntity* ent )
{
    /*
     * Find nearest node to this point.
     * Run through all the nodes in the graph and find the nearest one, then test that the
     * entity can actually get to this point
     */
    ...
    Report( object_reg, "NavrulesFPS - FindNearestNode");
    ...
    for( int i=0; i < iCount; i++)
    {
        ...
        if ( (fclosestdist == -1) || (fdist < fclosestdist) )
        {
            ...
            // use traverselink to check whether the given entity can traverse the link
        ...
        //Report( object_reg, "Node %d - sq. dist: %f, closest sq. dist: %f", i, fdist, fclosestdist);
