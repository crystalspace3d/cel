...
bool celNewStateRewardFactory::Load (iDocumentNode* node)
{
  ...
  if (!entity_name_par)
  {
    csReport (type->object_reg, CS_REPORTER_SEVERITY_ERROR,
      "cel.questreward.debugprint",
      "'entity_name' attribute is missing for the newstate reward!");
    return false;
  }
  return true;
}
...
void celNewStateRewardFactory::SetEntityNameParameter (const char* entity_name)
{
...
void celNewStateReward::Reward ()
{
  if (!quest)
  {
    ...
    csWeakRef<iPcQuest> pcquest = CEL_QUERY_PROPCLASS_ENT (entity, iPcQuest);
    if (!pcquest) return;
